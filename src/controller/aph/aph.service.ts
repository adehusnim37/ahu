...
export class APHService {
    ...
    async getAllAPH(
        ...
        const query: Prisma.pemeriksaanAPHFindManyArgs = {
            ...
            orderBy: sortBy
                ? {
                    [sortBy]: isSortAscending === 'true' ? 'asc' : 'desc',
                }
                : undefined,
    ...
    async updateAPH(id: string, data: pemeriksaanAPHModel, userId): Promise<pemeriksaanAPHModel> {
        ...
        const unchangeableFields = ['id', 'userId', 'namaPemohon', 'recInsert', 'isSubmit', 'dateSubmit', 'status', 'isVerified', 'dateVerified', 'CatatanTolak'];
        ...
        console.log(existingRecord.nosurat)
        console.log(data.nosurat)
